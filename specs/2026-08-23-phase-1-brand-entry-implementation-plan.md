...
- [x] 为三个方向分别绘制纯矢量标记和中文组合预览。
- [x] 在 16、24、32、64、128、256 像素下生成渲染样张。
- [x] 检查单色、浅色背景、深色背景和灰度情况下的辨识度。
- [x] 反向检查是否像机器人、聊天气泡、厂商标志、无限符号或通用 AI 火花。
- [x] 提交：`feat(brand): 生成三个非吉祥物 Logo 候选`。
...
- [x] 以五项强制维度和可访问性、可维护性两个附加维度进行逐项评分。
- [x] 每个候选列出至少三个失败模式，不允许只写优点。
- [x] 对最高分方案再做相似性、缩放、负形、线宽和中文字标压力测试。
- [x] 若最高分方案存在高严重度问题，先修复并重新渲染，不带问题进入正式目录。
- [x] 记录选择理由、被否决方向和证据文件。
- [x] 提交：`docs(review): 完成 Logo 候选对抗评审`。
...
- [x] 先扩展测试，要求正式资产与评审赢家一致、全部登记 Manifest、PNG 尺寸正确。
- [x] 运行测试并确认缺少资产时失败。
- [x] 从评审赢家派生核心标记、中文组合标和 1280×640 社交预览。
- [x] 使用确定性渲染脚本生成 PNG，不把渲染依赖加入项目依赖。
- [x] 运行品牌、视觉和 PNG 尺寸测试。
- [x] 打开渲染结果目视检查文字截断、像素糊化、负形粘连、对比度和窄屏效果。
- [x] 提交：`feat(brand): 发布正式中文品牌资产`。
...
- [x] 先增加 README 契约失败测试：必须使用正式组合标、中文主标题、六条主线、状态说明、证据边界和 Node 24 验证；不得出现旧标语、旧仓库名、英文入口或未审课程链接。
- [x] 运行测试并确认旧 README 失败。
- [x] 重写 README，使用状态表展示未完成主线但不链接为正式课程。
- [x] 运行品牌、链接、样式和敏感信息检查。
- [x] 反向检查“完整”“覆盖所有”“生产可用”等过度承诺，以及隐性双主线表述。
- [x] 提交：`docs(readme): 重建 Agent Harness 中文公共入口`。
...
- [x] 先测试 `start` 文章类型、入口内容契约和正式导航状态限制。
- [x] 运行测试并确认当前元数据模型不认识总入口。
- [x] 为总入口增加专用文章类型，要求定位、概念图、阅读路径、状态解释、证据方法、验证入口和边界说明。
- [x] 写成可独立理解的完整入口，不使用一两句话占位；本阶段状态设为 `reviewed`。
- [x] README 正式导航只链接这一篇已复核入口；六条主线仍只显示状态。
- [x] 运行分析、内容、导航、链接和样式检查。
- [x] 提交：`docs(entry): 发布中文总入口`。
...
- [x] 先增加失败测试，禁止英文 README、旧 Logo 文件、旧仓库标识和旧“双 Harness”标语回到公开树。
- [x] 运行测试并确认旧文件触发失败。
- [x] 删除旧文件并修复所有相对链接。
- [x] 用 `rg` 检查旧文件名、旧仓库名、英文入口和旧标语零残留；历史规格中的明确否定说明不计为公开残留。
- [x] 运行项目文件、链接、视觉和可移植性检查。
- [x] 提交：`refactor(repo): 移除旧英文入口与旧视觉`。
...
- [x] 从已提交基线运行 Node 24 聚合检查和全部单元测试。
- [x] 逐项审计品牌契约、三个候选、评审证据、正式资产、PNG、README、总入口、旧文件清理和远端元数据目标。
- [x] 再次目视检查三种尺寸、浅深背景、README 顶图和 Social preview。
- [x] 主动寻找：品牌通用化、机器人残影、英文自然语言、状态夸大、死链接、未登记 SVG、绝对路径、远端提前变更和旧双主线残留。
- [x] 修复全部高严重度发现并重跑验证；中低风险必须明确记录去向。
- [x] 写入复核记录；只有记录通过后才勾选总路线阶段 1。
- [x] 提交：`chore(review): 完成阶段 1 品牌与入口复核`。
