...
- [x] 先写失败测试：`reviewed` 基础课少于 2600 字、10 段、正式中文 SVG 或 `Claim:` 引用时必须失败。
- [x] 运行测试并确认现有契约不足以阻断这些反例。
- [x] 最小升级 foundation 内容契约，不影响 Harness、角色、实验和附录规则。
- [x] 从 12 个锁定来源中建立六篇文章的候选源码、上游测试、官方文档和实验清单；只记录真实存在的路径。
- [x] 对每篇至少准备两个不同证据面，并标注哪些只能支持项目例子、哪些可以支撑共同抽象。
- [x] 反向检查证据清单是否把单个实现扩大成行业通则，或把文件名当成行为证明。
- [x] 提交：`feat(foundations): 建立共同基础内容与证据契约`。
...
- [x] 选择至少两个主线实现的真实循环入口和终止路径，分别核对源码与测试。
- [x] 用一个真实最小输入展示内容回复、工具调用、结果回送、继续循环和最终终止。
- [x] 区分模型响应、Harness Turn、工具执行 Attempt 和完整任务 Run，不混用统计单位。
- [x] 绘制时序图，明确哪些箭头由模型触发、哪些由 Harness 决策、哪些是外部副作用。
- [x] 注册继续或终止语义 Claim，并把平台或模式条件写入限定字段。
- [x] 运行全部局部门禁和渲染复核。
- [x] 提交：`docs(foundations): 发布一次真实 Agent Turn`。
...
- [x] 核对至少两种实现中 System/Developer/User 内容、工具 Schema、工具调用和工具结果的真实数据结构。
- [x] 给出一组可读 JSON 输入、工具调用和工具结果，解释身份、调用 ID、顺序和错误字段。
- [x] 说明流式增量、最终消息、结构化内容块和协议适配的边界，避免把某个 Provider 格式当成通用格式。
- [x] 绘制数据流图，显示序列化、Provider 适配、工具执行和结果重新进入上下文的位置。
- [x] 注册工具结果必须关联并重新进入模型上下文的限定 Claim。
- [x] 运行全部局部门禁和渲染复核。
- [x] 提交：`docs(foundations): 发布模型与工具数据契约`。
...
- [x] 分别锁定工具暴露、参数校验、策略决策、用户审批、OS/容器隔离和真实副作用的证据。
- [x] 用一个危险命令例子展示“工具存在”“策略允许”“用户批准”“Sandbox 可执行”四个条件如何组合。
- [x] 明确权限提示不是隔离，Sandbox 不是授权，Hook 也不能自动等于安全边界。
- [x] 绘制四层安全边界图，并对默认、可选、扩展、外部和未知能力使用文字标签。
- [x] 注册“权限决策不等于沙箱隔离”Claim；如果是跨实现抽象，使用 D 并列出证据链。
- [x] 运行全部局部门禁和渲染复核。
- [x] 提交：`docs(foundations): 发布工具权限与沙箱基础`。
...
- [x] 先写失败测试：六篇基础课未全部 `reviewed` 时不得批量进入正式导航；旧双主线入口不得继续作为公开概念入口。
- [x] README 和总入口加入六篇课程链接，并把共同基础状态更新为 6/6 `reviewed`。
- [x] 删除或收口已被新基础课完整替代的旧概念和 Eval 入口，修复所有相对链接与旧覆盖率假设。
- [x] 保留仍将在六条项目主线中重审的旧源码文章，但继续排除在正式导航之外。
- [x] 搜索旧“双 Harness 并列课程”承诺、旧文件名和过期入口零公开残留。
- [x] 运行导航、链接、项目文件、覆盖率、文风和聚合检查。
- [x] 提交：`docs(navigation): 开放共同基础课程`。
...
- [x] 逐篇审计解释深度、最小例子、Claim、限定条件、自检答案、链接和中文图示。
- [x] 打开六张正式图，检查窄屏、深浅背景、文字截断、箭头、字体和颜色依赖。
- [x] 主动寻找：把单个项目外推成通则、模型与 Harness 混淆、权限与隔离混淆、Session 与 Context 混淆、Trial 分母漂移、Feedback 与发布评测混淆。
- [x] 修复全部高严重度发现；中低风险明确记录后续去向。
- [x] 复核通过后勾选总路线阶段 2，并在提交后重跑聚合检查。
- [x] 提交：`chore(review): 完成阶段 2 共同基础复核`。
