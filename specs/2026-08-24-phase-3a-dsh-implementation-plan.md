...
- [x] 打开全部 DSH 正式图，检查中文、截断、箭头、颜色依赖和证据绑定。
- [x] 主动寻找：旧文直接搬运、设计记录数量崇拜、单个平台外推、安全边界折叠、反馈等同奖励、Benchmark 等同发布评测。
- [x] 修复全部高严重度发现，记录中低风险后续项。
- [x] 新建阶段复核记录，勾选总路线阶段 3A，提交后再跑聚合检查。
- [x] 提交：`chore(review): 完成阶段 3A DSH 主线复核`。
