...
- [ ] 先写失败测试：一级主线入口缺少系统架构图、端到端流程图、课程状态表或 Claim 时必须失败。
- [ ] 为九篇课程建立锁定源码、上游测试、旧长文覆盖和计划 Claim 清单，所有路径必须真实存在。
- [ ] 明确现有十五篇长文到九篇新课的逐项映射，任何旧页不得无去向删除。
- [ ] 反向检查：目录名、设计记录数量和旧 `reviewed` 状态都不能冒充行为证据。
- [ ] 运行单元测试与局部门禁。
- [ ] 提交：`feat(dsh): 建立主线发布契约与迁移证据清单`。
...
- [ ] 解释锁定版本、许可证、公开源码边界、课程状态和阅读顺序。
- [ ] 用真实输入、事件、工具结果和最终产物走完一次任务，不用伪 API 冒充上游格式。
- [ ] 架构图覆盖 bundle、Cordis、Agent、LLM、工具、Session、产品表面与评测接入。
- [ ] 流程图区分模型触发、Harness 决策、真实副作用与终止。
- [ ] 渲染两图并检查中文、窄屏、箭头和边界。
- [ ] 提交：`docs(dsh): 发布主线入口与真实任务全景`。
...
- [ ] 完成 `08-verification-design-limits.md`、中文证据闭环图和验证 Claim。
- [ ] 覆盖 invariant、测试层、文档同步、设计记录、未验证表面和已知限制。
- [ ] 不把上游自验证或本仓库门禁扩大成生产就绪。
- [ ] 提交：`docs(dsh): 发布自验证与设计限制课程`。

### Task 11：旧 DSH 长文退场与正式导航

- [ ] 对照迁移清单确认十五篇旧文每项内容已有新去向。
- [ ] 删除 `docs/deep/dsh-*.md`，修复旧 A 系列、附录和研究记录中的相对链接。
- [ ] README 和总入口把 DSH 状态更新为 `reviewed`，整批加入 DSH 主线入口，不提前开放其他五条主线。
- [ ] 增加失败测试：DSH 九篇未全部发布时，主线入口不得宣称完整；扩展样本不得进入正式导航。
- [ ] 运行链接、导航、项目文件、内容、Claim、视觉和聚合检查。
- [ ] 提交：`docs(navigation): 开放 DSH 一级主线`。
