...
- [ ] 新增比较矩阵、角色导航和实验记录的失败测试；矩阵每个非空单元必须引用存在的 Claim，禁止总分字段。
- [ ] 建立五个生命周期矩阵数据文件，六条主线每个单元明确能力状态、证据等级、条件、Claim 和不可比较项。
- [ ] 聚合门禁纳入新检查并提交：`test(comparison): 锁定跨主线证据契约`。

## Task 2：运行边界、配置与模型输入

- [ ] 覆盖入口/服务边界、配置合并、项目指令、系统提示、模型目录与真实可用条件。
- [ ] 新增中文数据流图，避免把同名配置资产写成等价实现。
- [ ] 提交：`docs(comparison): 发布运行配置与模型输入比较`。

## Task 3：循环、工具与执行

- [ ] 比较一次 Turn 的控制对象、工具可见性、参数校验、批处理、结果回送、停止与任务成功边界。
- [ ] 新增中文状态图，明确自然停止、工具继续、压缩和失败分支。
- [ ] 提交：`docs(comparison): 发布循环工具与执行比较`。

## Task 4：权限、状态与恢复

- [ ] 比较审批/确认、应用策略、操作系统隔离、持久 Session、模型 Context、有损摘要、快照与外部副作用。
- [ ] 新增中文分层图，禁止把权限询问、历史或回退写成统一安全事务。
- [ ] 提交：`docs(comparison): 发布权限状态与恢复比较`。

## Task 5：编排、协议与表面

- [ ] 比较子任务/子代理、Skill、Plugin、Hook、MCP、服务协议、终端、网页、桌面与编辑器投影。
- [ ] 新增中文边界图，区分发现、启用、连接、模型可见、执行和客户端呈现。
- [ ] 提交：`docs(comparison): 发布编排协议与表面比较`。
...
- [ ] 四篇角色文章分别建立决策问题、至少三步工作流、风险边界和可执行验收清单。
- [ ] 每项建议至少链接一个已复核机制页、比较页或实验页，不重复制造浅层事实。
- [ ] 提交：`docs(roles): 发布四条决策工作流`。

## Task 8：控制变量与独立评测实验

- [ ] 创建不需要付费模型的本地控制实验：以机器可读夹具验证 Trial 分母、Attempt 恢复、产品失败不重试成通过和 Artifact 血缘。
- [ ] 创建最小独立评分管线实验：固定 Scorer 版本，输出分数、理由、不可判定与证据引用；RewardAdapter 只在语义完整时启用。
- [ ] 保存输入、命令、环境、结果和失败记录，实验页面只声明实际执行范围。
- [ ] 提交：`test(labs): 发布控制变量与独立评测实验`。

## Task 9：原子导航与旧页退出

- [ ] 根 README 和总入口一次性开放五篇比较、四条角色路径和两项实验；新增缺篇、降级与零链接失败测试。
- [ ] 删除旧 `a1` 至 `a10`、`for-product.md` 和 `for-ops.md`，更新所有站内链接；不保留旧版本比较或兼容入口。
- [ ] 旧附录暂不删除，在阶段 6 迁入正式 `docs/appendix/` 并复核。
- [ ] 提交：`docs(navigation): 开放比较角色与实验`。
