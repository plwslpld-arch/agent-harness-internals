...
公开契约里，允许结果可以携带改写后的工具输入和权限更新建议；拒绝结果可以携带原因，并选择是否中断整个会话。
...
这张路径图中，开源 SDK 能证明后半段控制协议；内部规则求值只能依据公开契约描述其边界，不能伪装成已读到 Claude Code 内部实现。
...
这一步说明 Transport 和权限决定器是两种职责：Transport 搬运消息，Query 才负责把协议事件分派给 Python 回调。
