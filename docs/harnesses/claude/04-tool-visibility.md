...
消息消费器已经能从类型化消息中分流 `ToolUse`，但看见工具请求还不等于知道它为何可用。为了回答这个问题，现在要回到配置层，沿工具集合、权限规则和系统能力一层层往下判断。
...
这段配置解决的是「给模型提供哪些内置工具」，还没有回答「调用时需不需要批准」「工具进程能访问哪些目录」「命令能不能被操作系统执行」。如果把这几个问题混在一起，权限设计几乎一定会出错。
...
可以把它想成进办公楼：工具集合是「楼层目录」，静态权限是「门禁白名单」，动态权限是「前台电话确认」，Hook 是「进出楼时触发的审计流程」，操作系统能力则是那扇门背后的房间是否真的存在——目录上有一个房间，不代表你已经获准进入。即使门禁已经放行，房间里的设备也不一定能用。
...
`tools=[]` 会明确关闭所有内置工具，而 `tools=None` 表示没有通过这个字段覆盖基础集合。至于 `allowed_tools=[]`，它只表示没有额外的自动放行规则，并不等于关闭全部工具。
...
这段源码的重要性不在于字段只有两行，而在于注释已经明确划界：`tools` 决定可用的基础工具集合，而 `allowed_tools` 决定哪些调用无需询问。因此，课程后面再看到「工具已允许」时，要先问这句话指的究竟是哪一层。

## 配置怎样变成进程参数

Python SDK 并不在 `ClaudeAgentOptions` 内部执行权限算法，它只把可公开观察到的配置转换为 CLI 参数，然后启动或连接 Claude Code CLI。这个交界处正好也是开源证据能够到达的产品边界。
...
空列表的处理很具体：实现会显式生成 `--tools ""`，因此不能把它理解为「省略参数」——这段源码给出了可以直接核对的行为：关闭内置工具由明确参数表达，无须猜测默认值。
...
这里的「有效允许规则」不总等于用户原样填写的 `allowed_tools`，因为 Skills 也可能在后续的合成过程中补充规则。
...
这说明排查权限时不能只打印调用方最初的 Options，因为真正传给 CLI 的是合成后的有效配置。后面的权限课程还会看到，裸 `Skill` 可能直接自动放行整个工具，从而让动态回调根本没有机会运行。

## 三个容易犯的错误

### 错误一：把 `allowed_tools` 当工具白名单

如果只写 `allowed_tools=["Read"]`，就不能据此断言模型只看得到 `Read`，因为要限制可见集合，还需要同时考虑 `tools` 或当前使用的预设。

### 错误二：把「自动允许」当「系统一定执行成功」

即使权限规则已经放行，目标文件仍可能不存在，进程也可能没有文件系统权限，命令还可能被 Sandbox 或操作系统拒绝。由于这些失败不属于同一层，权限结论和执行结果必须分别记录。

### 错误三：从 SDK 参数反推闭源产品内部算法

公开源码能证明 SDK 生成了哪些参数，以及控制协议怎样收发请求，但它无法证明 Claude Code 内部用什么类、按什么私有函数调用顺序求值。所以本文会在这里保留产品边界，不补画无法核对的虚构源码链路。
...
它表达了三层意图：模型只获得只读内置工具，这三类只读调用无需逐次询问，同时高风险工具会另外得到拒绝规则。然而，因为 Settings、MCP 工具、Hook、Sandbox 和进程权限也会影响最终行为，所以这段配置仍然构不成完整的安全证明。
