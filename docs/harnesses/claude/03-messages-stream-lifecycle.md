...
SDK 输出的不只是一段 Assistant 文本，一次运行中还可能出现用户消息、Assistant 内容块、系统事件、增量事件、Hook 事件和 Result。若把它们全部经过 `str(message)` 打印后丢掉类型，工具调用、错误、成本、权限拒绝和会话标识都会变成难以复盘的日志碎片。类型不能丢。
...
比如 Result 的 subtype 显示成功，但 Agent 忘记修改文件，此时协议可以完整结束，任务却仍然失败。反过来，即使文件碰巧正确，只要控制通道异常退出，这次运行就不应被记成干净成功。这两者不能混淆。
...
Hook 事件会先于普通 SystemMessage 分支处理，这个顺序说明，只按顶层 `type=system` 分类会丢失更具体的事件语义。因此，可靠的消费者应该优先匹配 SDK 提供的类型，不要自己重新发明一套字符串判断。

## AssistantMessage 是内容块容器

一条 Assistant 消息可能同时包含文本块和工具调用块，而模型提出 ToolUse 只代表「请求执行」——不代表权限已允许或工具已经成功。为了完成最终审计，应用必须用关联字段把 ToolUse 与后续工具结果、权限轨迹和副作用接起来。关联字段不能丢。
...
`parent_tool_use_id` 可以表示这条消息从某个工具或子流程派生，而 `session_id` 与 `uuid` 适合持久化关联，`stop_reason` 则用来解释模型响应为什么停止。这些字段承担的作用不同，所以不要把它们全部压缩成一条显示文本。

## 增量事件和完整消息不要重复计数

开启部分消息后，应用可能同时收到 `StreamEvent` 和完整 `AssistantMessage`。前者适合实时 UI 和首字延迟，而后者适合最终 Transcript 与评测，因此如果先拼接所有增量、再追加完整消息，文本和 Token 统计就会重复。重复就在这里发生。
...
网络或进程异常时，可能只有部分增量而没有完整消息，此时 Artifact 应该标记 incomplete，不能把已拼接文本伪装成正常的完整响应。轨迹并不完整。
...
Result 很适合回答「这次协议运行消耗多少轮、是否报告错误、为什么终止」，但它没有检查仓库文件是否正确。因此，正确的 Eval 应在 Result 之外读取工作区差异、测试输出和约束违规。协议终态不是 Eval。
...
如果把第一个 `receive_response()` 结束误认为连接已关闭，第二轮逻辑就会被错误地放到新 Session 或新进程。迭代结束了，连接仍然存在。

## 建议的 Artifact 结构

如果只保存最后一段文字，评测时就无法还原中间过程，因此一个最小可评测产物可以包含以下内容。
...
前三项来自协议轨迹，而工作区差异和测试结果来自执行环境。Scorer 只有同时读取这两类信息，才能区分下面几种情况：
...
这个示例只展示类型分流，距离完整审计器还少了不少工作。真实系统还需要关联 ToolResult、Hook、权限决定、异常和工作区副作用，并且对其中的敏感内容做脱敏处理。

类型化轨迹明确了 ToolUse 只是执行请求，因此下一篇要回到 Options，拆开模型可见的工具集合与真正的执行权限。
