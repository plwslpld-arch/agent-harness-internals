...
OpenCode 以服务化 Session 为核心：Project 和 Config 选择运行上下文，Provider 建立模型接口，Session Prompt 驱动主循环，Processor 消费模型流和工具结果，Server 与 Protocol 把同一核心暴露给多个客户端。

![OpenCode 系统地图](../../../assets/diagrams/opencode/system-architecture.svg)

## 这条课程适合谁

如果你想理解一个 Agent 核心怎样作为服务被 TUI、Desktop、Web 和协议客户端共享，可以选择 OpenCode。它比最小 Loop 多出 Project、Server 和持久化边界，适合已经完成基础导读的读者。
...
客户端在某个 Project 中创建或选择 Session，Prompt 主链组合消息、Agent 配置和工具，再向 Provider 发起模型请求。Processor 处理流式事件和工具状态，权限询问通过 Session 事件到达客户端，结果继续写回 Session。
...
图中客户端负责展示询问和结果，真正的 Session 状态仍由服务核心维护。多客户端不等于复制多份 Agent Loop。
...
Permission 规则和用户询问控制产品行为，不自动构成操作系统 Sandbox。真实隔离仍取决于宿主或另行配置的执行环境。
...
从客户端选择 Server/Directory 开始，说明 Project/Config/Provider 怎样建立实例，Session Prompt 怎样驱动 LLM 与 Processor，Permission Question 怎样到达客户端，Message/Part 怎样持久化，Compaction/Revert 如何改变有效历史，多种产品表面又如何从同一 Server 读取事实。

最后分别判断 Share 同步、OpenTelemetry Span、Session Idle 和独立测试能证明什么。它们都可以关联同一 Session，但不能互相替代。
