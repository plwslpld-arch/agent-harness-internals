...
Share 把 Session 投影复制到外部查看服务；Telemetry 记录运行观察；上游测试保护特定代码契约；独立 Eval 才按固定任务检查 Artifact。四者可以复用同一 Session 身份，但回答的问题不同。
...
自动分享是数据出站行为，必须由清晰配置控制；子 Session 是否包含敏感工具输出也要考虑。

### 分享不是普通 UI 功能

Share Worker 持续复制 Message、Part 和 Diff，可能包含源码、命令输出、路径和模型推理片段。它需要与 Provider 调用一样接受数据分类、脱敏、删除和审计要求。一个公开链接能打开，只证明远端存在某种投影，不证明同步完整，也不证明删除生命周期正确。
...
分享链接存在不代表远端副本与本地完全同步；删除本地 Session 也不应被假设自动删除外部副本，必须核对 API 结果。
...
源码 Telemetry 没有 Rubric、Score 或 Release Decision 字段。没有 Error Span 也不能证明修改正确。
...
训练信号若来自这些结果，应通过版本化 Reward Adapter；模型选择和最终发布仍用隔离任务。Share View、Session Idle、Tool Success、用户 Always Allow 或上游单元测试都不能单独替代这一步。

## 回到运费任务

Share 页面可以帮助人复盘模型和工具过程，Telemetry 可以说明延迟和成本，上游测试可以保护 Processor 或 Permission 的代码契约；只有外部 Scorer 在冻结工作区运行目标测试并检查 Diff，才能判断这次修复是否满足任务。四种证据互补，但不能互相冒名。
...
不一定。应把分享同步记录为可观测性或产品功能失败，同时仍可依据本地冻结 Artifact 判定代码任务结果。若任务本身要求「成功创建分享」，同步失败才属于产品结果失败。评分口径必须由任务定义，而不是由某个辅助系统是否绿色决定。
