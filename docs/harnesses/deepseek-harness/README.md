...
| 01 | 启动与 preset | 编写中 | bundle、profile、Cordis 与 preset 怎样得到有效插件树？ |
| 02 | Prompt、Context 与缓存 | 待发布 | 模型输入如何装配，哪些变化会破坏稳定前缀？ |
| 03 | 循环、模型与工具 | 待发布 | 流式响应、工具闭环、重试、取消和终止如何协作？ |
| 04 | 工具安全 | 待发布 | Guard、审批、策略与平台 Sandbox 怎样分层？ |
| 05 | 会话与压缩 | 待发布 | 原始事件、派生 Context、摘要、Spill 与恢复谁是权威？ |
| 06 | 编排与扩展 | 待发布 | Plan、Goal、Subagent、Workflow、Skill 与运行时代码怎样接入？ |
| 07 | 产品表面与评测 | 待发布 | Web、ACP、SDK、反馈、遥测和 Eval 接口怎样共享核心？ |
