...
学习地图已经把第一次源码阅读压缩成三步：观察一个任务、找到核心入口、跟完一条调用链；沿着其中的「输入」继续追，第一站就是模型真正收到的内容——但在看到请求之前，必须先看配置和项目指令怎样抵达这里。第一次读 Codex，最容易把「配置文件里的内容」直接等同于「发给模型的 Prompt」。实际上中间至少经过三层：磁盘与命令行提供候选配置，线程启动时把它们解析成有效配置，每次 Turn 再把指令、历史、工具和用户输入组装成模型请求。
