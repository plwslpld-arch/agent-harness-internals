...
学习地图已经把第一次源码阅读压缩成三步：观察一个任务、找到核心入口、跟完一条调用链，而沿着其中的「输入」继续追，第一站就是模型真正收到的内容——但在看到请求之前，必须先看配置和项目指令怎样抵达这里。第一次读 Codex，最容易把「配置文件里的内容」直接等同于「发给模型的 Prompt」。实际上中间至少经过三层：磁盘与命令行提供候选配置，线程启动时把它们解析成有效配置，每次 Turn 再把指令、历史、工具和用户输入组装成模型请求。
...
Rust 的 `Option::or` 选择左侧第一个 `Some`。因为「命令行优先」只对使用这种合并方式的字段成立，所以要判断某个具体选项，仍然得看它自己的解析代码。

## AGENTS.md 是项目上下文，不是无限长系统提示

Codex 会沿工作目录层级发现项目指令，并受最大字节数与后备文件名限制。进入更深的目录后，新发现的指令会作为新上下文进入后续 Turn，而已经存在于历史里的旧消息，不会因为当前目录改变而被倒写。
...
到这里，配置和上下文已被投影为一次模型请求——但「一个请求」还不是 Codex 的完整运行边界。下一篇先分清 Thread、Session、Task 与 Turn，才能判断返回的 Tool Call 落在哪一层生命周期里。
