...
「目录存在」「被发现」「已启用」「模型可见」「获准执行」「执行成功」是六个不同检查点。
...
Skill 被列在目录中不代表其正文已经进入 Context；正文进入 Context也不代表依赖工具连接成功。
...
不能因为所有 Hook 都写在同一配置文件里，就假定都能阻断操作。判断一个 Hook 的权限，必须读对应事件的消费代码。
...
Plugin 卸载时也要按来源撤销能力，不能只删目录而让旧 Tool Catalog 留在活动 Session。

## MCP 与 Connector：连接成功后还要决定模型是否可见

MCP Manager 维护连接与 Tool Catalog Cache；Connector Runtime 还要处理策略、鉴权和运行时投影。外部服务器返回一个工具，不代表当前模型就能调用它。
...
连接、目录发现、模型可见、调用授权和远端执行是五个独立状态。错误信息应指出失败在哪一层。

## Code Mode：把多次工具往返收进一个程序

Code Mode 向模型暴露一个代码执行入口，程序内部通过绑定调用现有工具。它适合读取多文件、聚合结果和带条件的批处理；它不应被理解为绕过 Tool Router 的宿主后门。
...
为何两种行为不同？普通模式的目标是优化工具使用，回退仍保持原能力；`code-mode-only` 的目标是限制模型只能走代码入口，暴露直接 Shell 会违反策略，所以只能拒绝。
