---
title: 设计记录导读：739 篇 Agent Note 里最值得读的那些
sources: [{"repo":"deepseek-harness","path":".agents/notes/README.md","commit":"b150a551b8d465e31e418e1b2eaf5e79bbb7d28e"}]
last_verified: 2026-08-23
status: reviewed
---

# 设计记录导读：739 篇 Agent Note 里最值得读的那些
...
这个语料库有 739 篇英文笔记。每篇是一个三件套：英文原文 `x.md`、中文镜像 `x.zh.md`、记录两侧配对状态的 `x.i18n.yaml`，所以落到磁盘上是 739 × 3 = 2,217 个文件。日期跨度 2026-06-11 到 2026-08-21，两个多月。本文做两件事：讲清它的组织规则，然后挑出二十篇（十四篇 implemented、三篇 rejected、三篇 proposed）逐篇说明它解决你什么疑问。二十篇不是「最重要的二十篇」的排名，是一份够用的入口。组织规则讲清楚之后，剩下的 719 篇你自己按路径捞比看别人的清单快。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| implemented | 143 | 186 | 94 | 74 | 48 | 14 | **559** |
| archived | 14 | 54 | 19 | 20 | 28 | 8 | **143** |
| proposed | 10 | 4 | 1 | 7 | 2 | 2 | **26** |
| rejected | — | 1 | — | — | 10 | — | **11** |
| 合计 | 167 | 245 | 114 | 101 | 88 | 24 | **739** |
...
739 篇里真正会改变你读源码方式的没那么多。下面挑十四篇，覆盖前面几篇正文用得最多的五个主题；每格给出「日期 · 状态」和一句话论点，路径都相对 `.agents/notes/`。这不是全集；同主题的其余笔记用第九节最后那条 `grep` 按关键词自己捞，路径本身就带着状态和日期，捞出来一眼能判断值不值得读。
...
## 七、上游 `docs/`：112 篇里哪些是机器写的

英文文档 112 篇、29,076 行。**15 篇是纯生成物**，文件头第一行就写着 `Generated by scripts/xxx.ts — do not edit by hand`（由某个脚本生成，别手工改）：

| 生成物 | 行数 | 内容 |
| --- | --- | --- |
| `docs/config-catalog.md` | 3,352 | 每个可加载包的 `config:` 声明逐字粘贴（JSDoc 一起），并交叉校验运行时 schema |
| `docs/tool-catalog.md` | 2,221 | 模型可见的全部工具 schema。生成器**真的把每个工具插件启动起来**读 `ctx.tools.schemas()`，因为 schema 静态不可知 |
| `docs/module-graph.md` | 1,691 | 包依赖图，从 `peerDependencies` 推导 |
| `docs/persistence-catalog.md` | 1,007 | 每种持久化产物 |
| `docs/capability-seams.md` | 488 | 全部 seam 的 Definition/Provider/Consumer 图 |
| `docs/agent-lifecycle.md` / `docs/tool-execution-pipeline.md` / `docs/event-producer-consumer.md` / `docs/graph-atlas.md` | 82 / 62 / 79 / 24 | turn-step 顺序图、工具流水线图、事件生产消费矩阵、图索引 |
| `docs/cordis-api/*.md` | 6 篇 | Cordis 框架 API |

另有 44 篇**含嵌入的生成区**，全部落在 `docs/subsystems/` 的 47 篇子系统页里（另外 3 篇没有生成区）。`<!-- BEGIN GENERATED cordis-surface -->`（这一行的意思就是「生成区从这里开始」）到 `END` 之间是从源码抽的 Cordis API 区，人只能写标记之外的部分。所有这些都由 `doc-sync` 里的 `--check` 门禁保证新鲜。

**人写的部分**里，按性价比排序值得读的：

- `docs/architecture.md`（131 行）：地图。短、准、有权威，但故意不讲实现细节。
...
- `docs/postmortem/`（5 篇）：真事故复盘。
- `docs/cookbook/`（9 篇）：加一个包 / 一个工具 / 一个 LLM 适配器的逐文件清单。
- `docs/subsystems/`（47 篇）：查类型和契约用，不适合通读。

### 上游文档和本仓库的分工

上游已经讲清楚的：**类型与契约**（subsystems）、**配置字段**（config-catalog）、**工具 schema**（tool-catalog）、**事件生产消费矩阵**、**包依赖图**、**每个包对 prompt/token/缓存的影响**（223 份 README 的 Model Experience 小节）、以及**每个决策的 why**（Agent Note）。这些都比本仓库全，需要精确答案时应该直接查上游。

上游按自己的文档标准**不会**写的：跨包的端到端机制叙事（一次请求从 inbox 到 wire 的逐步走读）、历史演进（被 slop 清单禁止）、横向对比、以及把 559 篇 implemented 笔记浓缩成的设计原则清单。这四块就是本仓库的位置。
...
**2. `rejected/` 只有十一篇，`implemented/` 有 559 篇，为什么本文说前者信息密度更高？**
