...
*这一篇讲给要给自己的 agent 加上下文压缩、或者在查「压缩之后为什么突然变贵」的人。读完你能回答：压缩什么时候触发、砍掉的那一段去了哪里、这次压缩本身要花多少钱。*

「上下文满了，把前面截掉一段」听起来是个纯本地操作，不花钱。

真做起来，砍掉的那段得先让模型写成摘要，也就是要多发一次 LLM 请求。这次请求的第一个 token 只要和刚才那次对话请求对不上，provider 的前缀缓存就整个作废：同一段历史，在对话最长最贵的时候被全价算了两遍。上游踩过这个坑，改法只有一句话，把摘要指令从请求开头挪到对话末尾。
...
**checkpoint（检查点）** 是压缩的产物：一条替代原文的摘要消息，模型从此只看得到它，看不到被它顶掉的那一段。下面四行取自上游 e2e 快照（`examples/headless-agent/tests/snapshots/compaction-recovery/session.jsonl:21-24`），是一次完整压缩事务在日志里留下的全部痕迹：
...
- `seq 20` 是账本：摘要正文、模型原始输出、被遮蔽（源码里叫 shadowed，指「被这次替换顶掉、模型从此看不见」的那些节点）的范围与 seq 列表、被遮蔽内容的估价（266 token）、实际路由到哪个 provider/model、辅助请求的 usage。这条事件**不上 surface**，模型看不到。**surface（表面）** 是事件日志的一个子视图，只有能进模型历史的那几类事件在上面；投影模型消息时只折叠 surface，日志里其余的事件是给人和工具看的。
- `seq 21` 才是模型看得见的东西：一条 `user/message`，`surfaceOp` 是 `{op:'replace', start:4, end:4}`，它把 surface 上的 `seq 4` 节点整个遮蔽掉，自己占据那个位置。**`surfaceOp`** 是往 surface 写东西时必须显式声明的操作，`append` 是追加，`replace` 是拿这条新事件把指定区间的节点整个遮蔽掉。压缩改写历史时一条事件都不删，只往日志尾部追加一条 replace。`sourceEventSeqs: [19,20,4]` 把锁、账本、被遮蔽节点全部引上。
- checkpoint 消息的正文结构是固定的三段：前言 + `<compacted-summary>` 开标签、摘要文本块、`</compacted-summary>` 闭标签。

`seq 21` 里那段英文前言翻过来是：「这是一份自动生成的 checkpoint，它把更早的一段对话压缩掉，好腾出上下文。把里面记下的内容当作既定背景，在它之上继续，不要复述一遍。直接从后面的消息接着做任务，不要理会这条 checkpoint 本身。」最后半句是给模型的行为约束，目的是让它把这条消息当背景资料，而不是当成一条需要回应的用户发言。
...
3. `tokenMeter.measure(session).totalTokens < thresholdTokens` → 返回 null，什么都不做。`tokenMeter` 是本地估算器，不问 provider，按固定启发式换算，它的偏差见本文「代价与失效点」第 4 条。
4. 超阈值了，**先跑模型无关的剪枝**（如果装了 `toolResultPruner`），重新测量；还是低于阈值就到此为止（`:306-312`）。剪枝就是把过长的工具输出砍掉中段，纯本地、不调模型，细节见本文「模型无关的剪枝」。这一步能完全避免掉那次花钱的摘要调用。
...
挂在同一个 waterfall 上，但只处理一种失败码（`packages/compaction/compaction-basic/src/index.ts:179-223`）。**waterfall（瀑布事件）** 是 Cordis 的环绕式中间件：监听器必须 `await next()` 才轮到下一个，返回值权威。挑失败码的那句判断就是第一行：
...
八步照顺序读：先校验区间（存在、有序、两端 tool 配对平衡），再查日志里有没有开放的 turn、有没有没配对的 `compaction/start`、最新的种子边界在哪，确认当前没有别的压缩在跑；然后写锁；接着测量、给被遮蔽的节点估价、拼出摘要请求，发那次 LLM 调用；回来再确认这期间 surface 没被人改动过；最后把 `compaction/summary` 和那条 replace 的 `user/message` 一起提交，放锁。
...
抛出的那句英文意思是「摘要并不比它遮蔽掉的内容更小」，括号里给出两个估价数字方便对账。注意比较的是**加了框架之后**的 checkpoint 消息估价，不是裸摘要：前言加上两个标签也要占 token，只按裸摘要算就可能压出一条更长的历史。压缩不缩小就不算压缩。
...
`system` 和 `tools` 直接取自最新的 `request/header`；消息是被遮蔽的 seq 逐个过 `deriveEventMessage`，也就是 `deriveMessages()` 折进主请求的**同一批对象**。块里那条英文注释解释的是为什么这里敢用非空断言：`shadowedSeqs` 存的是当前 surface 上的 seq，每一个都必然是合法的日志下标。
...
（你现在是这个 AI 编码助手的压缩引擎。把**上面**那段对话压成一份结构化的 checkpoint，要让另一个模型能凭它接着干活，关键上下文一点不丢。

严格按下面的 Markdown 结构输出：每一节都保留，顺序不变。用短条目，别写成大段散文。某一节没内容就写 `(none)`，绝不能把整节删掉。）

指令里 `ABOVE`（上面）这个词有分量。**摘要器（summarizer）** 默认就是对话自己那个模型，被这条指令临时改了任务（`summarizationProvider` / `summarizationModel` 空着时沿用对话的路由，见上面的参数表）。它要读的历史原样摆在这条指令前面，和主对话请求里那份逐字相同，所以不需要另外准备一份被拍平的 transcript。

八节结构固定（`packages/compaction/compaction-basic/src/summarizer.ts:36-58`）：Primary Request and Intent / Key Technical Concepts / Files and Code / Errors and Fixes / Pending Jobs / Current Work / Next Step / Critical Context。八个英文标题对应的中文是：

| 小节 | 让模型写什么 |
| --- | --- |
| Primary Request and Intent | 用户最初的目标和后来变过的目标；措辞本身要紧时逐字引用 |
| Key Technical Concepts | 这次任务涉及的技术、框架、模式和约定 |
| Files and Code | 精确路径，以及这个文件为什么要紧、改了什么关键片段 |
| Errors and Fixes | 出过什么错、怎么解决的，连带相关的用户反馈 |
| Pending Jobs | 用户明确要求过、但还没做完的事 |
| Current Work | 压缩发生的那一刻，手头正在做的到底是哪一步 |
| Next Step | 下一个动作，只写一个，且必须紧贴最近一次请求；没有就写 `(none)` |
| Critical Context | 做过的决定和理由、约束、用户偏好、悬而未决的问题、继续干活需要的数据 |

上面这张表只是给你看的对照，模板里的小节标题是英文原文，模型照着它逐节往下填。五条规则（`packages/compaction/compaction-basic/src/summarizer.ts:60-65`）：写简洁的英文工程散文，保留精确的路径、命令、错误串、标识符、数值、函数签名；忠实记录用户反馈尤其是纠正；**不要提到这次摘要请求或上下文被压缩过**；只输出 checkpoint 文本，不调用任何工具；已有的 `<compacted-summary>` 块是上一个 checkpoint，不要原样抄下来，而要合并成一份统一的摘要。
...
（provider 是按请求开头那串 token 做缓存的，第一个 token 一变，比如换了个 system prompt，整段缓存前缀就全废。于是每压缩一次，同一段被重放的历史都要按全价付两遍 prompt 处理费：一遍给触发压力的那次对话请求，一遍给这次摘要调用。缓存恰恰在对话最长的时候失效。）

翻译成人话就是：压缩本来是省钱的动作，旧实现却让它在最贵的时刻多烧一整份历史。
...
- 「省掉 `tools`（反正摘要器不会调工具）」。被否，原话是 "tool schemas are part of the cached token sequence; omitting them misaligns every following token and defeats reuse"（工具 schema 本身就是被缓存的那串 token 的一部分，省掉它们会让后面每一个 token 全部错位，复用彻底失效）。**即使不调用也必须带上**。
...
这行标记的意思是「工具结果的中段被剪掉了」，它留在模型看得见的位置，模型读到的就是「这里被剪过」，而不是一份看上去完整、实际上少了一截的输出。
...
9. **`compaction/summary` 与其 `user/message` 的相邻性是契约而非机制**。类型注释写了 "The replacement MUST be appended synchronously right after this event"（替换事件必须同步地、紧跟在这条事件后面追加）（`packages/compaction/compaction/src/types.ts:78-79`），但没有运行时断言强制它。谁写了别的插件在这两条之间插一脚，编译期和运行期都不会拦。
...
**1. 为什么摘要指令要放在对话末尾？给摘要器单独写一个 system prompt 不是更清楚吗？**

因为 provider 是按请求开头那串 token 匹配缓存的，而 system 槽正是它看的第一段。换掉 system prompt，从第一个 token 就分叉，后面写得再对也复用不上。旧实现就是这么做的，代价是每次压缩都要把整段历史全价处理两遍：一遍给触发压力的对话请求，一遍给摘要调用，而这偏偏发生在对话最长的时候。挪到末尾之后，辅助请求 = 主请求的 system + tools + 前 N 条消息 + 一条新指令，是主请求的真前缀，前面全部命中。

同一份设计记录里被否掉的四个方案（`.agents/notes/implemented/bug-fix/2026-07-21-compaction-summary-prefix-cache-reuse.md:29-32`）都栽在同一件事上，其中最反直觉的是「摘要器又不调工具，那就别带 `tools` 了」：少一份工具 schema，后面每个 token 的位置全错，复用照样归零。

**2. 压缩的指令里为什么要专门写一条「不要提这次摘要请求，也不要提上下文被压缩过」？**

因为摘要落地之后不是一份报告，而是 surface 上一条冒充历史的 `user/message`。它前面那段 `CHECKPOINT_PREAMBLE` 已经告诉后续的模型：把这段当既定背景，直接往下做，不要理会这条 checkpoint。要是摘要正文自己再来一句「以上是被压缩的对话」，模型就会把「刚发生了一次压缩」当成对话内容去回应，而不是当成背景。

**3. 什么情况下压缩不但没省钱，反而每一轮都在全价重付前缀？**

自动压缩选区间时永远从 `surfaceNodes[0]` 开始（`packages/compaction/compaction-basic/src/region.ts:98-133` 的第 5 步），落地的是一条新的头部 checkpoint。所以下一次主对话请求从第一条消息起就和上次不同，miss 从位置 0 算起，保住的只有 system 和 tools。压得越勤，重写头部和「摘要自己上一次的摘要」这两笔损耗越大，上游提案对此的自评是「the head checkpoint is rewritten every pass…」（头部那条 checkpoint 每压一次就被重写一次）。

另有两种是主动放弃复用：手动 `/compact` 压中段时，被压区间不是请求头，重放的前缀对不上；配了和对话不同的 `summarizationProvider` / `summarizationModel` 时，路由都换了，缓存域自然也换了。这两种是部署方的显式取舍。
