...
Checkpoint 管文件恢复，Core 配置规定，根 Agent Run（Agent 运行）开始时可以捕获一份可恢复的工作区 Snapshot。内置实现用 Git stash 和私有 ref 来保存，也允许调用方提供自定义创建函数，而源码把默认值明确写成 `false`，所以你得主动启用 Checkpoint，它才会工作。
