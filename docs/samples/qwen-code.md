...
`TurnBoundaryCompactionEngine` 在 `turn_complete` 或 `turn_error` 处折叠当前轮次积累的事件。连续文本和思考块合并，工具调用序列归约到最终状态，瞬态信号被丢弃，不同事件类型的相对顺序保留。目标是让回放日志规模随对话轮次数增长，而不是随每个流式令牌增长。
...
Serve Bridge 还暴露改变会话批准模式的工具。它允许 plan、default、auto-edit、auto 与 yolo 五种值，但在 `allowGlobalScope` 未开启时，明确拒绝 auto-edit、auto、yolo 和持久化变更。只有显式开放全局作用域，才把请求交给 DaemonClient。
...
实时 Journal 与回放片段的接缝发生在轮次完成或错误边界。引擎在轮次内接收增量事件，按类型合并文本与思考、归约工具状态并保留跨类型顺序；随后把片段放入受字节和事件预算限制的窗口。实现要记录压缩前后数量、丢弃类型、截断锚点和当前 Cursor，客户端才能知道恢复到的是有损投影而非完整 Trace。

回放窗口与多客户端的接缝是快照加增量。客户端先取得窗口快照和锚点，再订阅新事件；断线重连使用 Cursor 去重，若所需位置已被淘汰就收到显式截断状态。高风险工具的原始调用、批准和结果应写入独立不可变审计流，不能依赖为 UI 优化的压缩 Journal。

Serve Bridge 与会话控制 API 的接缝由 `allowGlobalScope` 守卫。请求先校验模式、持久化意图和作用域，默认只允许较窄变化；开放全局后仍需绑定调用者、目标 Session、旧值、新值和有效期。模式变化只改变应用批准策略，工具执行器与 OS 隔离继续独立判断。
...
对其他主线，这个样本提出两个通用问题：长驻 Harness 向多个客户端提供会话时，回放如何限界；权限模式能否被协议表面远程扩大。Codex、Claude、pi、OpenCode 和 DeepSeek Harness 需要按各自实现回答，不能直接套用 Qwen Code 的字段或默认值。

## 什么时候值得采用

轮次边界压缩适合事件量远大于轮次数、客户端需要快速重连且 UI 不要求逐令牌审计的长驻服务。调试流式解析、合规取证或高风险工具时，需要并行保留更高保真证据；窗口预算越小，恢复成本与不可见历史越大，应按任务风险配置。

Bridge 作用域门禁适合防止普通远端工具扩大自动批准和持久化设置，但它只保护这一入口。直接 CLI、其他 SDK、配置文件与组织策略仍需分别核对。本文不因与 Gemini CLI 结构相似就外推语义，也不评价 Qwen Code 全部产品表面；可迁移的是有损回放声明、快照 Cursor 和权限扩大审计。
