...
六方循环都可以用「候选行动—结算—新观察—控制决定」来提问。模型输出提供候选文本或工具意图；Harness 把流式事件归并成完整调用，进行名称、参数与策略处理；执行器产生带关联的结果；控制器再判断继续、压缩、等待、取消或停止。这个抽象不把一次模型响应、一个工具批次、一个产品 Turn 和一个 Eval Trial 当成同一计数单位。

比较记录的最小对象是 `Call`、`Settlement` 和 `ControlOutcome`。Call 保存最终 Schema 版本、调用 ID 与完成参数；Settlement 保存是否进入执行器、环境输出、副作用引用和结果回送位置；ControlOutcome 保存谁作出的继续或终止决定以及仍待处理的状态。只记录最终助手文本，会同时丢掉三类对象。

完成语义必须按所有者分层。Provider Finish Reason 归模型响应，工具批次完成归调度器，Result Message 或 Session Idle 归协议或会话，进程退出归宿主，Verdict 归独立 Scorer。横向矩阵只比较这些状态怎样连接，不把任一层的「完成」翻译成统一的任务成功。

并行和恢复使用同一不变量：每个结果引用唯一调用，未提交副作用不能伪装成功，已提交但回执丢失不能直接重放。不同项目可以选择串行、批次、事件归约或远端协议；只要调用、结算与控制决定可追踪，就能在共同抽象下比较而不抹平实现差异。
...
为一个新的 Harness 接入确定性模型夹具：第一次响应发出两个工具调用，第二次在看到结果后输出终止文本。两个工具都写入同一临时文件，但让第二个调用先完成。保存原始流、归并后的 Call、执行开始与提交、回送给模型的工具结果、ControlOutcome 和最终文件 Hash。

接着注入三种故障。第一种在参数流中途截断，要求零副作用；第二种在文件提交后、结果持久化前中断，恢复时先核验文件和幂等键；第三种让循环自然停止但写入错误值，由独立 Scorer 判失败。每种故障都要说明属于响应、结算、恢复还是评测层，不能统一记成异常。

交付物是一份按调用 ID 连接的事件图、三份故障记录和两个最终 Verdict。验收时交换两个结果的完成顺序，关联仍应正确；删除某个工具结果，下一次模型请求必须被阻断或明确降级；循环自然停止的两个案例必须得到不同任务判定。这个练习验证状态机，不比较模型聪明程度。
