...
把 Turn 设计成显式状态机，是为了避免传输事件替代业务状态。流式响应可以因为网络、重连或 Provider 适配产生任意数量的分片；如果循环用「收到最后一个文本块」作为完成条件，工具参数尚未闭合、后台审批仍在等待或 follow-up 已排队时就可能提前终止。状态机只在所需输入都闭合后迁移，因此不依赖界面一次显示了多少行。

工具调用标识承担因果连接，而不是装饰字段。模型意图、策略判断、执行尝试、结果消息和下一次模型请求都引用同一标识，Harness 才能证明某段观察来自哪次行动。并行调用时尤其如此：完成顺序可以与请求顺序不同，若只靠数组位置，某个错误结果可能被回送给错误的调用，模型随后作出的修正就失去可解释性。

继续与终止由 Harness 决定，是因为模型只看得到请求里提供的状态。模型可以给出停止原因，却不知道宿主是否还有排队输入、工具是否发出终止信号、用户是否取消或策略是否等待确认。Harness 必须把这些外部事实合并为稳定终态，同时把项目特有 Hook 明确标成扩展决定，不能伪装成模型自身行为。

最后，循环恢复与 Eval 恢复必须分开。截断工具参数被转成错误观察后再次采样，属于同一任务内部的控制流；基础设施故障导致 Eval 新建 Attempt，则属于统计与恢复契约。共用一个「重试次数」会让产品失败被基础设施语义掩盖，也会让一次 Turn 的局部修正错误改变 Trial 分母。
...
可以先实现一个只支持单工具、串行执行的教学循环，再用失败注入验证状态，而不是一开始加入并行、压缩和多客户端。最小步骤如下：

1. 为交互 Turn 创建稳定标识，保存用户输入并进入 `sampling`；每次模型请求再分配独立的响应标识。
2. 收齐模型事件后解析完整内容；若存在工具调用，先创建 `requested` 记录，校验名称、参数与策略，禁止从流式半成品直接执行。
3. 获准后执行工具并产生带原调用标识的 `tool_result`；拒绝或校验失败也产生结果，但必须标明从未进入执行器。
4. 把结果写入下一次模型请求；结合取消、终止信号、排队输入与停止原因迁移状态，只有进入稳定终态才发出 `turn_end`。

```ts
type TurnState =
  | { tag: 'sampling'; responseId: string }
  | { tag: 'awaiting_tool'; callId: string }
  | { tag: 'executing_tool'; callId: string }
  | { tag: 'sampling_after_tool'; callId: string }
  | { tag: 'completed'; outcome: 'reply' | 'failed' | 'cancelled' };

function shouldContinue(state: TurnState): boolean {
  return state.tag !== 'completed';
}
```

这个伪代码故意不把 Provider 的 `finishReason` 存成最终 outcome。适配器可以把它作为响应事实写入事件，控制器再结合工具、审批与宿主状态判断下一步。生产实现还需要超时、并行批次、幂等提交、背压和恢复，但这些都应扩展状态，而不是绕过状态机直接插入副作用。
...
为上面的 `echo` 循环准备四份录制响应。正常样本先请求工具再回答 `done`；截断样本只给出半个参数；拒绝样本请求一个策略禁止的工具；取消样本在执行开始后收到取消信号。不要调用活模型，使每轮输入和事件顺序可以重复。

每轮执行后核对五个不变量：工具结果总能回到先前调用；被拒绝的调用没有执行开始；截断参数没有产生副作用；取消后没有新的模型采样；稳定终态之后没有事件追加。再把流式分片数量从一个改成十个，确认模型响应与 Turn 计数不变。若计数改变，说明实现仍把传输边界当成业务边界。

练习交付物应是一份事件 JSONL、一张按调用标识连接的状态迁移表和四个断言结果。额外挑战是加入两个并行工具，让完成顺序反转；Harness 可以按完成顺序记录环境事件，但回送模型的关联必须保持正确。此练习只验证教学循环的状态与关联，不证明锁定项目具有相同并行语义。
