...
「空」不是偷懒，是被门禁强制的表态。`scripts/verify-package-invariants.ts` 会用 TypeScript AST 解析每个 `invariant.ts`：空的 `install` 函数体，如果它的声明语句里没有 `No runtime invariant:` 这个字符串，直接失败（`scripts/package-invariants.ts:265-275`）；非空的 `install`，如果第二个参数（那个 `fail` 报告器）不存在、或者存在但函数体里从没被引用，也失败（`scripts/package-invariants.ts:278-285`）。换句话说，**你不能写一个看起来在检查、实际上永远不会报错的 invariant**。
...
| `llm-mock-server` | 一个可编排的 OpenAI 兼容 HTTP/SSE 服务器，按到达顺序消费预设行为。**无需 key 就能测真适配器**——不是 mock 掉 adapter，而是 mock 掉 provider，于是 SSE 解析、重试退避、超时都跑的是真代码。 |
...
这一节想说的是：**门禁能证明的东西是有边界的**。219 个 invariant、268k 行测试、28 个文档门禁，合起来仍然挡不住「测试和产品走了两条不同的加载路径」这类错误——只有把真实入口路径本身变成被测对象才行。
