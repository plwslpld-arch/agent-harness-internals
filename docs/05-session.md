...
这一层是 dsh 里我认为最值得学的部分：**它把「模型看到的历史」和「真实发生过什么」显式拆成两件事**，然后用类型系统和运行时不变量保证两者不会悄悄分叉。
...
`SurfaceIntent` 的注释把规则写死了：`surfaceOp` 在**产生消息的事件上必填，在 log-only 事件上禁止**。
...
`start` 和 `end` 都是闭区间，且都必须是**当前 surface 里存在的节点**；`start === end` 表示替换单个节点。
...
`types.ts:422` 的注释是我在这个仓库里读到的最好的一段设计说明： `evidence: code`
...
工具分两种状态（`:13`、`:16`），而且**恢复策略是用自然语言写给模型看的**（`:104-105` 原文）： `evidence: code`
...
**这个设计值得单独说。** 恢复策略没有做成一个状态码让上层去解释，而是直接把判断依据写成模型能读的指令：「只有只读或幂等才重试；可能有副作用就先核对外部状态或问用户」。模型是这条恢复路径上的决策者之一，所以策略得说给它听。
...
这是本文最该记住的一条。`TOOL_OUTCOME_UNKNOWN` 意味着**工具可能已经执行了一半**——文件可能已写、请求可能已发、钱可能已扣。自动重试会把一次不确定变成一次确定的重复副作用。
