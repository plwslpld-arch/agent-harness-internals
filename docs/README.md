...
PDF 与本站同源，由 CI 从 MkDocs 导航里的同一套 Markdown 生成，因此不会与网页版走散。

## 姊妹项目

这套教材只回答「模型的下一步意图怎样变成可控制的真实动作」。至于动作做完之后由谁判定它
真的做对了，那是另一套系统的问题，写在 [Eval Harness 源码内核](https://plwslpld-arch.github.io/eval-harness-internals/) 里：
任务与数据集怎样定义、运行器怎样并发与重试、评分与统计怎样得出结论、发布门禁凭什么放行。
两套教材共用同一种读法，都从锁定源码的行号锚点出发。
