...
for (const source of selectedSources) {
  if (source.fetchPolicy === 'restricted' && !includeRestricted) {
    console.log(`跳过 ${source.id}（受限来源；需要时使用 --include-restricted）`);
    continue;
  }
  ...
  try {
    ...
    console.log(`获取 ${source.id} @ ${locked.commit}`);
  ...
  console.log(`来源就绪 ${source.id} @ ${locked.commit.slice(0, 12)}`);
}
