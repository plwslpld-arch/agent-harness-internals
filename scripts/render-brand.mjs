...
export async function renderBrand({ projectRoot = root } = {}) {
  ...
  const input = join(projectRoot, 'docs', 'assets', 'brand', 'social-preview.svg');
  const output = join(projectRoot, 'docs', 'assets', 'brand', 'social-preview.png');
  ...
  return { output: 'docs/assets/brand/social-preview.png', width: result.width, height: result.height };
}
