...
| [Claude Agent SDK for Python](https://github.com/anthropics/claude-agent-sdk-python) | Claude Code 的官方契约面：消息、hook、权限与工具 schema | MIT；只据 SDK 契约与官方文档，不外推闭源实现 |
...
Claude Code 本体不开源。本仓库只把 Claude Agent SDK 的类型与协议当作可核对的契约面；契约面之外只依据公开官方文档，不使用泄露的 prompt 转储，也不把 SDK 行为外推成闭源内部实现。
