...
> 当前状态：仓库正在覆盖式重建。治理、证据门禁、正式品牌和共同基础 6/6 已完成阶段复核；DSH 与 Codex 均以 9/9 达到 `reviewed`，其余四条主线仍处于 `outline`。未完成内容不会以链接冒充成品。
...
正式课程导航只会出现 `reviewed` 和 `verified` 页面。六篇共同基础、DSH 九篇和 Codex 九篇已经整批完成复核并开放；其余四条项目主线继续显示状态，但达到发布门槛前不提供课程链接。
